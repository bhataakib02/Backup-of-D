...
const IDSFunction = () => {
  ...
  const [masterInput, setMasterInput] = useState({ networkData: '', pcapFile: null });
  ...
  useEffect(() => {
    if (location.state) {
      ...
      setInput(location.state.masterInput.networkData || '');
    }
  }, [location.state]);
        ...
        case 'detection':
          result = {
            ...
            data: {
              ...
              attackTypes: ['DDoS', 'Port Scan', 'Intrusion'],
        ...
        case 'intel':
          result = {
            ...
            data: {
              ...
              confidence: Math.random() * 100,
              sources: ['Threat Feeds', 'Honeypots', 'Sandboxes'],
              categories: ['Malware', 'APT', 'Botnet']
            },
            executionTime: executionTime
          };
          break;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            functionData.category === 'network' ? 'bg-neon-cyan/20 text-neon-cyan' :
            functionData.category === 'detection' ? 'bg-danger-red/20 text-danger-red' :
            functionData.category === 'ml' ? 'bg-quantum-purple/20 text-quantum-purple' :
            functionData.category === 'realtime' ? 'bg-plasma-green/20 text-plasma-green' :
            functionData.category === 'storage' ? 'bg-fusion-orange/20 text-fusion-orange' :
            functionData.category === 'performance' ? 'bg-electric-blue/20 text-electric-blue' :
            functionData.category === 'intel' ? 'bg-warning-yellow/20 text-warning-yellow' :
      ...
      <div className="card-cyber">
        ...
        <div className="space-y-4">
          ...
          {masterInput.networkData && (
            <div>
              <label className="block text-sm font-medium text-text-primary mb-2">
                Master Network Data Input
              </label>
              <textarea
                value={masterInput.networkData}
                readOnly
          ...
          {masterInput.pcapFile && (
            <div>
              <label className="block text-sm font-medium text-text-primary mb-2">
                Master PCAP File Input
              </label>
              <div className="p-3 bg-dark-surface/50 rounded-lg border border-dark-border">
                <p className="text-sm text-text-primary">{masterInput.pcapFile.name}</p>
                <p className="text-xs text-text-muted">Size: {(masterInput.pcapFile.size / 1024).toFixed(2)} KB</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              placeholder={functionData.inputExample || "Enter network data for this function..."}
