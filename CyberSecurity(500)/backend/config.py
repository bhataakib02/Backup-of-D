...
class Config:
    ...
    # OIDC SSO (placeholders)
    OIDC_ISSUER_URL = os.getenv('OIDC_ISSUER_URL')
    OIDC_CLIENT_ID = os.getenv('OIDC_CLIENT_ID')
    OIDC_CLIENT_SECRET = os.getenv('OIDC_CLIENT_SECRET')
    OIDC_REDIRECT_URI = os.getenv('OIDC_REDIRECT_URI', 'http://localhost:5000/api/auth/oidc/callback')
...
def validate_config(env_name: str, config_cls) -> None:
    """Validate critical settings, especially in production.
    ...
    missing = []
    # Secrets
    if not config_cls.SECRET_KEY or config_cls.SECRET_KEY.startswith('dev-'):
        missing.append('SECRET_KEY')
    if not config_cls.JWT_SECRET_KEY or 'change-in-production' in config_cls.JWT_SECRET_KEY:
        missing.append('JWT_SECRET')
    # Database
    if not config_cls.SQLALCHEMY_DATABASE_URI:
        missing.append('DATABASE_URL')
    # Redis (soft requirement but recommended)
    if not config_cls.REDIS_URL:
        missing.append('REDIS_URL')

    if missing:
        raise RuntimeError(f"Missing critical environment settings for production: {', '.join(missing)}")
