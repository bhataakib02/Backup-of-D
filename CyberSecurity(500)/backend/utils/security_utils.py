...
def zero_trust_required(fn):
    """Simple zero-trust check: require device and posture headers."""
    @wraps(fn)
    def wrapper(*args, **kwargs):
        device_id = request.headers.get('X-Device-ID')
        posture = request.headers.get('X-Device-Posture')  # compliant/noncompliant
        if not device_id or posture != 'compliant':
            from flask import jsonify
            return jsonify({'error': 'Zero-trust check failed', 'details': 'Device posture noncompliant or missing'}), 403
